...
export const EVENTS: Event[] = [
  {
    ...
    agentLabel: "Research",
    color: "#7489A3",
    label: "Indexing Halo filings, launch cadence, Series C messaging",
    shadowFrac: 0.012,
    todayFrac: 0.005,
  },
  {
    ...
    agentLabel: "Research",
    color: "#7489A3",
    label: "Pulling space and satellite awards calendar — 187 events surfaced",
    shadowFrac: 0.05,
    todayFrac: 0.05,
  },
  {
    ...
    agentLabel: "Analysis",
    color: "#977BA1",
    label: "Scoring each event against the 'broadband for the unconnected' angle",
    shadowFrac: 0.12,
    todayFrac: 0.14,
  },
  {
    ...
    agentLabel: "Analysis",
    color: "#977BA1",
    label: "Analyzing five years of category winners for selection patterns",
    shadowFrac: 0.2,
    todayFrac: 0.24,
  },
  {
    ...
    agentLabel: "Strategy",
    color: "#88A374",
    label: "Pruning eligibility — 187 to 38 viable opportunities",
    shadowFrac: 0.29,
    todayFrac: 0.33,
  },
  {
    ...
    agentLabel: "Strategy",
    color: "#88A374",
    label: "Ranking on prestige, fit, deadline, and narrative momentum",
    shadowFrac: 0.38,
    todayFrac: 0.43,
  },
  {
    ...
    agentLabel: "Plan",
    color: "#7489A3",
    label: "Shortlisting top 12, mapping deadlines and submission specs",
    shadowFrac: 0.48,
    todayFrac: 0.52,
  },
  {
    ...
    color: "#B27A53",
    label:
      "Drafting top four — Fast Company WCI, Edison, SATELLITE 2026, GLOMO",
    shadowFrac: 0.59,
    todayFrac: 0.62,
  },
  {
    ...
    agentLabel: "Writer",
    color: "#B27A53",
    label: "Tailoring each draft to the award's historical winner profile",
    shadowFrac: 0.71,
    todayFrac: 0.76,
  },
  {
    ...
    agentLabel: "Writer",
    color: "#B27A53",
    label: "Cross-referencing Halo customer wins, deployment metrics, and ESG",
    shadowFrac: 0.83,
    todayFrac: 0.88,
  },
  {
    ...
    agentLabel: "Strategy",
    color: "#88A374",
    label: "Compiling open questions for CFO sign-off and customer quotes",
    shadowFrac: 0.95,
    todayFrac: 0.95,
  },
  {
    ...
    agentLabel: "Sent",
    color: "#88A374",
    label: "Staging the awards and events package for Maya's review",
    shadowFrac: 1.0,
    todayFrac: 1.0,
    outcome: true,
  },
...
export const CYCLE_MS = 28000;
export const SHADOW_DONE_T = 0.0714; // ~2s real-time
export const TODAY_DONE_T = 0.92;

const SHADOW_TRACK_WIDTH_PCT = 22;

export function findLatestFired(t: number, frac: (ev: Event) => number): Event | null {
...
const TODAY_TOTAL_DAYS = 5 + 2 / 24; // 5d 2h end value
const SHADOW_TOTAL_SEC = 4 * 60 + 36; // 4m 36s end value

export function formatDays(t: number): string {
  const totalMin = Math.max(0, Math.min(1, t)) * TODAY_TOTAL_DAYS * 24 * 60;
...
export function formatMinSec(t: number): string {
  const sec = Math.max(0, Math.min(1, t)) * SHADOW_TOTAL_SEC;
...
export function TimelineRow({
  ...
  // Shadow rides the UV blue; Today is bone-on-warm-ink, no chroma.
  const ridgeColor = accent ? "#7489A3" : "#5a4a3a";
  ...
  const staffingColorClass = accent ? "text-[#7489A3] font-normal" : "text-[#B27A53] font-normal";
  const staffingLabelColorClass = accent ? "text-[#7489A3]/70" : "text-[#B27A53]/70";
    ...
    <div className="flex items-baseline gap-6">
      <div className="flex flex-col items-start">
        <span
          ...
          style={{ fontSize: metricFontSize ?? "clamp(2.5rem, 5vw, 4rem)" }}
        >
          {staffing}
        </span>
        <span
    ...
    <div className="flex flex-col gap-1">
      <span
        className={
          ...
          (accent ? "text-[#7489A3]" : "text-foreground/55")
        }
      >
        {tag}
      </span>
...
function CaptionBlock({
  ...
  return (
    <div className="inline-flex items-center gap-2 w-fit font-mono text-[10px] uppercase tracking-[0.18em] tabular-nums whitespace-nowrap">
      <span className={accent ? "text-[#7489A3]" : "text-muted-foreground/80"}>
        {accent
          ? done
            ? "Done"
            : "Live"
 ...
 */
export function ShadowAgentPanel({ t }: { t: number }) {
  ...
  return (
    <div className="flex flex-col h-[460px]">
      {/* Inbound profile card — sender header + message body, matches
          the design language of ProgramAgentDemo. */}
      ...
      {/* Stream — flat, no border; top + bottom mask fade so it dissolves
          into the panel like the ProgramAgentDemo timeline. */}
      <div
        ref={scrollRef}
        ...
        <ol className="relative">
          {fired.map((ev, i) => (
            <li
              ...
              className="craft-rise relative pb-6 pl-6"
            >
              {/* Per-row dashed connector — starts below the dot, runs to
                  the row's bottom. The next row's dot at top breaks the
                  line, matching ProgramAgentDemo. */}
              <span
                aria-hidden="true"
                className="absolute left-[5px] top-[18px] bottom-0 border-l border-dashed border-border"
              />
              {/* Step dot — uniform gray for chain consistency. */}
              <span className="absolute left-[2px] top-[7px] w-1.5 h-1.5 rounded-full bg-foreground/55" />
